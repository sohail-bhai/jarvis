...
## Vision
The ultimate goal of this project is to build a self-learning AI agent assistant named JARVIS. 
...
## Gap Analysis: Current System vs. True JARVIS
- **Understanding:** We currently use rigid `if/else` substring matching. A true JARVIS uses Natural Language Processing (NLP) to understand complex, multi-turn intents.
- **Action Space:** We currently run hardcoded OS scripts. A true JARVIS can interact with any software by "seeing" the screen and using the mouse/keyboard programmatically.
- **Memory & Learning:** We currently have no memory between sessions. A true JARVIS learns from mistakes and remembers user preferences permanently.
- **Privacy & Connectivity:** We currently rely on Google Speech Recognition (requires internet). A true JARVIS processes voice, text, and thoughts entirely locally.
...
## Current Progress
- **Phase 1, Step 1 (Completed):** Successfully integrated `ollama` as the local LLM engine in `ai_brain.py`. JARVIS can now think locally without internet access for basic conversational requests.
...
- **Phase 1, Step 4 (Completed):** Added short-term conversation memory. JARVIS now remembers the last 10 interactions in a session, allowing for follow-up questions and contextual awareness.
...
- **Phase 2, Step 3 (Completed):** Upgraded `ai_brain.py` with an **Agent Loop**. JARVIS can now perform multi-step actions (e.g., calling a tool, checking the result, and deciding what to do next up to 5 times) before talking back to the user.
- **Phase 3, Step 1 & 2 (Completed):** Added `assistant/memory.py` leveraging a local `data/memory.json` store. Gave JARVIS a `remember_fact` tool and injected the contents of `memory.json` into its System Prompt on startup for permanent long-term context.
